...
print "rsem-calculate-expression --strand-specific --paired-end --star --star-path /users/jrwhite/jbin --star-gzipped-read-file --sort-bam-by-coordinate --output-genome-bam -p $max_cores $FQ_PAIRS{$id}{R1} $FQ_PAIRS{$id}{R2} $REF_DIR $OUT_DIR/rsem/$id/$id";
 `rsem-calculate-expression --strand-specific --paired-end --star --star-path /users/jrwhite/jbin --star-gzipped-read-file --sort-bam-by-coordinate --output-genome-bam -p $max_cores $FQ_PAIRS{$id}{R1} $FQ_PAIRS{$id}{R2} $REF_DIR $OUT_DIR/rsem/$id/$id`;
