...
##*********************************************************************
use vars qw/$opt_d $opt_o $opt_i $opt_c/;
getopts("d:i:o:c:");
my $usage =
".USAGE.   
rs_mito_pipeline.pl -d < directory of paired end fastq.gz files > -i < file index number to process > -o < output dir >

.DESCRIPTION.
This pipeline performs filtering of all human genome, followed by alignment to the human 
mitocondrial genome. User submits a directory containing paired-end fastq.gz files e.g. 

<prefix>_R1.fastq.gz and <prefix>_R2.fastq.gz

and the pipeline will operate on each pair independently, storing the results in the user provided
output directory.
...
`java -jar /users/jrwhite/jlib/VarScan.v2.4.2.jar mpileup2cns $OUT_DIR/$id/MT_alignment/cleaned.sorted.mpileup --min-coverage $vmincoverage --min-reads2 $vminreads2 --min-avg-qual $vminavgqual --min-var-freq $vminvarfreq --min-freq-for-hom $vminfreqforhom > $OUT_DIR/$id/MT_alignment/VarScan2.output.txt`;

# also VCF:
`java -jar /users/jrwhite/jlib/VarScan.v2.4.2.jar mpileup2cns $OUT_DIR/$id/MT_alignment/cleaned.sorted.mpileup --min-coverage $vmincoverage --min-reads2 $vminreads2 --min-avg-qual $vminavgqual --min-var-freq $vminvarfreq --min-freq-for-hom $vminfreqforhom --output-vcf > $OUT_DIR/$id/MT_alignment/VarScan2.output.vcf`;

# VEP:
`perl /users/jrwhite/jlib/ensembl-tools-release-87/scripts/variant_effect_predictor/variant_effect_predictor.pl --offline -i $OUT_DIR/$id/MT_alignment/VarScan2.output.vcf --cache --dir /dcl01/scharpf/data/jrwhite/PROJECTS/.vep --output_file $OUT_DIR/$id/MT_alignment/VarScan2.output.vep`;
